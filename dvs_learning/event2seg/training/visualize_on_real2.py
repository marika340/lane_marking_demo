...
def pick_frame_for_visual(pred, event_vis_small, mask_small, meta):
    ...
    if tp == "ViT":
        ...
        idx = 0 # you can change this to pick a different frame in the batch/time
        ev0 = event_vis_small[idx]
        gt0 = mask_small[idx]
        # print(f"[viz_eval] ViT pred shape: {pred.shape}") # 128, 1, 60, 90
        ...
        print(f"[viz_eval] ViT pr0 shape: {pr0.shape}") # 1, 60, 90
        return ev0, pr0, gt0
...
def visualize_and_save(ev0, pr0, gt0, out_dir="pred_debug", prefix="sample0"):
    ...
    # pred is logits -> sigoid applied in _to_np_img
