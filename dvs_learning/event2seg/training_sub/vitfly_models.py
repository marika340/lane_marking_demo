...
@brief: This module contains the models that were used in the paper "Utilizing vision
 transformer models for end-to-end vision-based quadrotor obstacle avoidance" by Bhattacharya, et. al
"""
...
class LSTMNetVIT(nn.Module):
    """
    ViT+LSTM Network (event-only version)
    """
    def __init__(self):
        super().__init__()
        self.encoder_blocks = nn.ModuleList([
        ...
        self.decoder = spectral_norm(nn.Linear(4608, 512))
        self.lstm = nn.LSTM(input_size=512, hidden_size=128, num_layers=3, dropout=0.1)
        # self.nn_fc2 = spectral_norm(nn.Linear(128, 3))
        self.nn_fc2 = nn.Sequential(
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 60 * 90),   # match output resolution
            nn.Sigmoid()              # optional: if you don’t use BCEWithLogitsLoss
        )
    ...
    def forward(self, X):
        # X: [event_frame] or [event_frame, hidden_state]
        x = refine_inputs(X)[0]
        ...
        if len(X) > 1:
            out, h = self.lstm(out, X[1])  # hidden state
        else:
            out, h = self.lstm(out)

        out = self.nn_fc2(out)           # [B, 60*90]
        out = out.view(-1, 1, 60, 90)    # reshape to [B, 1, H, W]
        return out, h
