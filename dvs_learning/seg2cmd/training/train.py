...
# the binarization threshold is 20 for 0-255 scale, and 0.08 for 0-1 scale
...
class TRAINER:
    def __init__(self, args):
        ...
        print("[DEBUG] args.dataset =", args.dataset)
        print("[DEBUG] args.dataset_range =", args.dataset_range)
        ...
        # (train_meta, train_masks, train_lengths, train_cmds, train_quats), \
        # (val_meta,   val_masks,   val_lengths,   val_cmds,   val_quats), \
        # is_png, _, (cmd_mean, cmd_std) = dataloader(
        #     data_dir=args.basedir,
        #     train_val_dirs=(train_dirs, val_dirs),
        #     seed=42
        # )

        # train_loader, val_loader, (cmd_mean, cmd_std) = dataloader(
        #     data_dir=args.basedir,
        #     val_split=0.1,          # or explicit (train_dirs, val_dirs) support if you want
        #     seq_len=16,
        #     batch_size=8,
        #     num_workers=4,
        #     seed=42
        ...
        with open(opj(self.workspace, "norm_stats.txt"), "w") as f:
            f.write("cmd_mean: " + " ".join(map(str, cmd_mean.tolist())) + "\n")
            f.write("cmd_std: " + " ".join(map(str, cmd_std.tolist())) + "\n")
            print("[INFO] Saved cmd_mean and cmd_std")

        # self.train_data, self.val_data = dataloader(data_dirs=dataset_list, val_split=0.1, batch_size=1, num_workers=4)

        collate = collate_fn if self.model_type == "SimpleCNN" else None

        # # --- wrap into Dataset classes ---
        # train_dataset = DrivingDataset(train_masks, train_cmds, train_meta, train_lengths,
        #                             seq_len=args.seq_len, mean=cmd_mean, std=cmd_std)
        # val_dataset   = DrivingDataset(val_masks, val_cmds, val_meta, val_lengths,
        #                             seq_len=args.seq_len, mean=cmd_mean, std=cmd_std)
        ...
        # self.val_data = DataLoader(val_dataset,
        #                         batch_size=args.batch_size,
        #                         shuffle=False,
        #                         num_workers=args.num_workers,
        #                         collate_fn=collate)
    ...
    def train(self):
        print(f"[TRAIN] Training for {self.epochs} epochs")
        ...
        for ep in range(self.epochs):
            ...
            for mask_batch, drive_batch in self.train_data:
                ...
                drive_batch = drive_batch.to(self.device)    # (B,5)
                ...
                if self.model_type == "SimpleCNN":
                    ...
                    # mask_batch: (B,1,H,W), drive_batch: (B,5)
                    ...
                    B, T, C, H, W = mask_batch.shape
                    mask_batch = mask_batch.view(B * T, C, H, W)
                    drive_batch = drive_batch.view(B * T, -1)
                    # mask_batch = F.interpolate(
                    #     mask_batch, size=(60, 90),
                    #     mode='bilinear', align_corners=False
                    # )
                    ...
                    X = [mask_batch]
                    pred, _ = self.model(X)   # (B,output_dim)
                    loss = F.mse_loss(pred, drive_batch)
                else:
                    ...
                    print(f"[DEBUG] train mask after threshold unique vals: {torch.unique(mask_batch)}")
                    X = [mask_batch]
                    pred, _ = self.model(X)   # (B,T,5)
                    # print(f"[DEBUG] pred shape: {pred.shape}, drive_batch shape: {drive_batch.shape}")
                    loss = F.mse_loss(pred, drive_batch)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for mask_batch, drive_batch in self.val_data:
                ...
                drive_batch = drive_batch.to(self.device)
                ...
                if self.model_type == "SimpleCNN":
                    ...
                    mask_batch = (mask_batch > 0.08).float()
                    X = [mask_batch]
                    pred, _ = self.model(X)   # (B,T,output_dim)
                    loss = F.mse_loss(pred, drive_batch)
    ...
    def _infer_model_type_from_keys(state_dict_keys):
    ...
    def load_from_checkpoint(self, checkpoint_path):
        ...
        inferred = _infer_model_type_from_keys(state_dict.keys())
    ...
    # def run_model(self, index=0, mode='val', return_inputs=False, seq_input=False, do_step=False):
    ...
    #         mask, drive = batch
    #         mask = mask.to(self.device)           # shape: (B, 1, H, W)
    #         drive = drive.to(self.device)         # shape: (B, 5)

    #         # Forward pass
    #         pred, extras = self.model(mask)

    #         if return_inputs:
    #             return None, (pred, extras), (mask, None, torch.ones_like(drive), (drive, None))
    #         else:
    #             return None, (pred, extras), None

    def run_model(self, mode='val', return_inputs=False, unnormalize=True):
        data_loader = self.val_data if mode == 'val' else self.train_data
        results = []

        for batch in data_loader:
            mask_seq, drive = batch  # mask_seq shape: (B, T, 1, H, W)
            mask_seq = mask_seq.to(self.device)
            drive = drive.to(self.device)
            
            if self.model_type == "SimpleCNN":
                # Already flattened by simplecnn_collate_fn:
                # mask_seq: (B,1,H,W), drive: (B,5)
                mask_seq = F.interpolate(mask_seq, size=(60, 90), mode='bilinear', align_corners=False)
                X = [mask_seq]
                pred, extras = self.model(X)   # (B,5)
            else:
                # Sequential models (e.g. LSTMNetVIT) expect (B,T,1,H,W)
                B, T, C, H, W = mask_seq.shape
                mask_seq = mask_seq.view(B * T, C, H, W)
                mask_seq = F.interpolate(mask_seq, size=(60, 90), mode='bilinear', align_corners=False)
            ...
            if unnormalize:
                pred = pred * self.cmd_std.to(pred.device) + self.cmd_mean.to(pred.device)
                drive = drive * self.cmd_std.to(drive.device) + self.cmd_mean.to(drive.device)
            ...
            if return_inputs:
                results.append((None, (pred, extras), (mask_seq, None, torch.ones_like(drive), (drive, None))))
            else:
...
def argparsing(filename=None):
    ...
    if filename is not None:
        ...
        default_config_files = [f'/home/{uname}/catkin_ws/src/lane_marking_demo/dvs_learning/seg2cmd/training/configs/train_config_seg2cmd.txt']
    ...
    # parser.add_argument('--dataset_range', type=int, nargs=2, help='Start and end index for dataset folder range', default=[110, 171])
    # parser.add_argument('--dataset_range', type=int, nargs=2, help='Start and end index for dataset folder range')
    ...
    parser.add_argument('--checkpoint_path', type=str, nargs='+', required=True, help='Path(s) to checkpoint(s)')
    ...
    parser.add_argument('--train_dir', type=str, default='132-135', help='Training data directory')
    parser.add_argument('--val_dir', type=str, default='152', help='Validation data directory')
