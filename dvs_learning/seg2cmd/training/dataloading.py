...
# ============================================================
# DrivingDataset: lazy loading from file paths + metadata
# ============================================================
class DrivingDataset(Dataset):
    def __init__(self, folders, seq_len=16, transform=None, cmd_mean=None, cmd_std=None):
        ...
        for folder in folders:
            ...
            if not os.path.exists(csv_file) or not os.path.exists(mask_dir):
            ...
            n = min(len(mask_files), traj_meta.shape[0])

            # build samples (start indices for sequences)
            for i in range(0, n - seq_len + 1, seq_len):
                self.samples.append((mask_files[i:i+seq_len], traj_meta[i:i+seq_len, 2:4]))  # use steering_angle + speed
    ...
    def __getitem__(self, idx):
        mask_files, cmds = self.samples[idx]
        mask_seq = []
        for f in mask_files:
...
def collate_fn(batch):
    all_images, all_cmds = zip(*batch)  # each is (T,1,H,W), (T,2)
    return torch.stack(all_images, dim=0), torch.stack(all_cmds, dim=0)
...
def dataloader(data_dir, val_split=0.1, seq_len=16, batch_size=8, num_workers=4, seed=None, train_val_dirs=None):
    ...
    # --- datasets ---
    train_dataset = DrivingDataset(train_folders, seq_len=seq_len, cmd_mean=cmd_mean, cmd_std=cmd_std)
    val_dataset   = DrivingDataset(val_folders,   seq_len=seq_len, cmd_mean=cmd_mean, cmd_std=cmd_std)
