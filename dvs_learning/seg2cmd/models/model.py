...
# -----------------------------------------------------------------------------
# LSTM + ViT encoder for segmentation-mask -> full AckermannDrive (5D)
# -----------------------------------------------------------------------------
# class LSTMNetVIT(nn.Module):
...
#         cat = torch.cat([up, ps], dim=1)  # (B,44,16,24)? Actually 32+12=44
...
#         dec_seq = dec.unsqueeze(0)      # (1, B, 512)
#         if hidden is None:
#             out_seq, h = self.lstm(dec_seq)
#         else:
#             out_seq, h = self.lstm(dec_seq, hidden)

#         # squeeze out the seq dimension, then final FC
#         out = out_seq.squeeze(0)        # (B,128)
...
#         mask_seq = X[0]  # (B, T, 1, 60, 90)
#         print(f"mask_seq shape: {mask_seq.shape}")
...
#             features.append(dec.unsqueeze(0))  # (1, B, 512)

#         # Stack features into a temporal sequence: (T, B, 512)
#         seq_input = torch.cat(features, dim=0)

#         # Feed into LSTM
#         if hidden is None:
#             out_seq, h = self.lstm(seq_input)  # (T, B, 128)
#         else:
#             out_seq, h = self.lstm(seq_input, hidden)

#         # Get final timestep output: (B, 128)
#         out = out_seq[-1]
...
#         self.nn_fc2 = spectral_norm(nn.Linear(256, 1))
#         self.up_sample = nn.Upsample(size=(16,24), mode='bilinear', align_corners=True)
#         self.pxShuffle = nn.PixelShuffle(upscale_factor=2)
#         self.down_sample = nn.Conv2d(48,12,3, padding = 1)
