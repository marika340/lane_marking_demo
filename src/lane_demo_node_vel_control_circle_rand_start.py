...
class LaneDemoNode(object):
    def __init__(self):
        rospy.init_node("lane_demo_node_vel_control_circle_rand_start", anonymous=False)
        ...
        # 1) Define your waypoints for a continuous circle track (x, y) in world frame
        r = 10.0        # radius of each loop
        n = 500        # total number of sample points around the circle
        cx, cy = 0.0, 0.0
        ...
        self.waypoints = [(r * np.cos(t), r * np.sin(t)) for t in theta]
        
        # np.save("/home/marikan/catkin_ws/src/lane_marking_demo/worlds/track_circle.npy", self.waypoints)
