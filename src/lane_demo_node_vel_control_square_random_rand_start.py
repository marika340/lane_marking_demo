...
class LaneDemoNode(object):
    def __init__(self):
        rospy.init_node("lane_demo_node_vel_control_square_random_rand_start", anonymous=False)
        ...
        # 1) Define your waypoints for a continuous square track (x, y) in world frame
        n = 500        # total number of sample points around the square
        cx, cy = 0.0, 0.0

        # 2) Lane-line half-width (offset from centerline)
        self.lane_half_width = 0.2  # total lane width = 1.0 m
        side_length = 10.0     # length of each straight section
        radius = random.uniform(0.5, 3.0) # radius for rounded corners
        points_per_segment = 50
        # Write radius in file
        radius_file = PathL.home() / "catkin_ws" / "src" / "lane_marking_demo" / "worlds" / "square_corner_radius.json"
        ...
        x = np.linspace(-side_length/2 + radius, side_length/2 - radius, points_per_segment)
        y = np.full_like(x, -side_length/2)
        waypoints += list(zip(x, y))

        # 2. Bottom-right corner (quarter circle, -90 to 0 degrees)
        theta = np.linspace(-math.pi/2, 0, points_per_segment)
        cx, cy = side_length/2 - radius, -side_length/2 + radius
        waypoints += [(cx + radius * math.cos(t), cy + radius * math.sin(t)) for t in theta]

        # 3. Right straight (bottom to top)
        y = np.linspace(-side_length/2 + radius, side_length/2 - radius, points_per_segment)
        x = np.full_like(y, side_length/2)
        waypoints += list(zip(x, y))

        # 4. Top-right corner (0 to 90 degrees)
        theta = np.linspace(0, math.pi/2, points_per_segment)
        cx, cy = side_length/2 - radius, side_length/2 - radius
        waypoints += [(cx + radius * math.cos(t), cy + radius * math.sin(t)) for t in theta]

        # 5. Top straight (right to left)
        x = np.linspace(side_length/2 - radius, -side_length/2 + radius, points_per_segment)
        y = np.full_like(x, side_length/2)
        waypoints += list(zip(x, y))

        # 6. Top-left corner (90 to 180 degrees)
        theta = np.linspace(math.pi/2, math.pi, points_per_segment)
        cx, cy = -side_length/2 + radius, side_length/2 - radius
        waypoints += [(cx + radius * math.cos(t), cy + radius * math.sin(t)) for t in theta]

        # 7. Left straight (top to bottom)
        y = np.linspace(side_length/2 - radius, -side_length/2 + radius, points_per_segment)
        x = np.full_like(y, -side_length/2)
        waypoints += list(zip(x, y))

        # 8. Bottom-left corner (180 to 270 degrees)
        theta = np.linspace(math.pi, 3*math.pi/2, points_per_segment)
        cx, cy = -side_length/2 + radius, -side_length/2 + radius
        waypoints += [(cx + radius * math.cos(t), cy + radius * math.sin(t)) for t in theta]

        self.waypoints = waypoints
        # np.save("/home/marikan/catkin_ws/src/lane_marking_demo/worlds/track_square_random.npy", self.waypoints)
        ...
        if pose is None:
            ...
            cam_x, cam_y, _ = pose
            # Pick globally closest waypoint once at start
            dists = [math.hypot(wp[0] - cam_x, wp[1] - cam_y) for wp in self.waypoints]
            self.time_index = int(np.argmin(dists))
            rospy.loginfo(f"Initialized time_index to {self.time_index}")
    ...
    def stop_vehicle(self):
        ...
        twist.angular.y = 0.0
        twist.angular.z = 0.0
        rospy.loginfo("Publishing stop command before shutdown...")
        self.cmd_vel_pub.publish(twist)
