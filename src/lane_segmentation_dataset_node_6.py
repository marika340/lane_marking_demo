...
class LaneSegmentationDatasetCollector:
    def __init__(self):
        rospy.init_node("lane_segmentation_dataset_node_6", anonymous=False)

        # Output directory
        # Automatically create a new dataset directory with incremented number
        base_dir = os.path.expanduser("~/catkin_ws/src/lane_marking_demo")
        dataset_prefix = "dataset"
        i = 110
        while os.path.exists(os.path.join(base_dir, f"{dataset_prefix}{i}")):
            i += 1

        self.output_dir = os.path.join(base_dir, f"{dataset_prefix}{i}")
        ...
        rospy.loginfo(f"Created new dataset folder: {self.output_dir}")
        ...
        # Stop condition
        self.has_left_start = False  # Track if vehicle has completed initial departure

        self.initial_pose = None
        self.shutdown_triggered = False
        self.done_threshold = 1.0  # meters
    ...
    def ready_callback(self, msg):
        if not self.ready and msg.data:
            rospy.loginfo("Received lane ready == True. Starting 10 second shutdown timer.")
            rospy.Timer(rospy.Duration(10.0), self.shutdown_after_timeout, oneshot=True)
    ...
    def model_states_callback(self, msg):
        if "camera_model" not in msg.name:
            return
        ...
        # Wait until vehicle goes far away first (e.g. to start lap)
        # if not self.has_left_start:
        #     if distance > 5.0:  # meters
        #         rospy.loginfo("Vehicle left initial area — tracking return for shutdown.")
        #         self.has_left_start = True
        #     return

        # Then wait for it to return to initial area
        # circle, square
        # if distance < self.done_threshold:
        #     rospy.loginfo("Vehicle returned to initial position. Shutting down dataset collection...")
        #     self.shutdown_triggered = True
        #     rospy.signal_shutdown("Episode complete")

        # figure 8
        # if not hasattr(self, "return_count"):
        #     self.return_count = 0

        # if distance < self.done_threshold:
        #     if self.has_left_start:
        #         self.return_count += 1
        #         rospy.loginfo(f"Return to start area detected. Count: {self.return_count}")
        #         self.has_left_start = False  # reset so it only counts again after leaving

        #         if self.return_count >= 2:
        #             rospy.loginfo("Vehicle returned to initial position twice. Shutting down dataset collection...")
        #             self.shutdown_triggered = True
        #             rospy.signal_shutdown("Episode complete")
        # else:
        #     if distance > 5.0:
    ...
    def shutdown_after_timeout(self, event):
        if not self.shutdown_triggered:
            rospy.loginfo("Timeout reached. Shutting down dataset collection...")
            self.shutdown_triggered = True
            rospy.signal_shutdown("Timeout after lane ready.")
