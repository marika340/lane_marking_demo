...
class MonitorReturn:
    ...
    def pose_callback(self, msg):
        ...
        rospy.loginfo_throttle(0.5, f"Distance to start: {dist:.2f} | Has left: {self.has_left_start} | Return count: {self.return_count}")
        ...
        if dist < self.threshold:
            if self.has_left_start:
                self.return_count += 1
                rospy.loginfo(f"Return to start detected. Count: {self.return_count}")
                self.has_left_start = False  # wait for next lap

                if self.return_count >= 2:
                    rospy.loginfo("Vehicle returned to start twice. Killing background processes.")
        ...
        elif dist > self.depart_threshold:
            self.has_left_start = True  # reset for another return loop
