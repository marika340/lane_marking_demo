...
class LaneSegmentationDatasetCollector:
    def __init__(self, track):
        rospy.init_node("lane_segmentation_dataset_node_5")

        # Output directory
        # Automatically create a new dataset directory with incremented number
        rospy.loginfo("[Collect] lane segmentation dataset node 5 started. ")
    ...
    def rgb_callback(self, msg):
        ...
        rospy.loginfo(f"[Collection] Received RGB at {timestamp_rgb}")
        if not self.ready:
            return
    ...
    def save_metadata(self):
        ...
        radius_file = Path.home() / "catkin_ws" / "src" / "lane_marking_demo" / "worlds" / "square_corner_radius.json"
        if radius_file.exists():
            with open(radius_file, "r") as f:
                radius_data = json.load(f)
            radius = radius_data.get("radius", 2.0)
        else:
            print("radius file doesn't exist")
...
if __name__ == '__main__':
    ...
    try:
        node = LaneSegmentationDatasetCollector(track=args.track)
        rospy.spin()
